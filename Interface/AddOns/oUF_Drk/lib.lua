...
local _, playerClass = UnitClass("player")
  ...
  -- local variables
  -----------------------------
  oUF.colors.power['MANA'] = {.2,.5,.8}
  oUF.colors.power['RAGE'] = {.8,.2,.2}
  oUF.colors.power['FOCUS'] = {.7,.45,.25}
  oUF.colors.power['ENERGY'] = {.7,.7,.3}
  oUF.colors.power['RUNIC_POWER'] = {.6,.6,.9}
...
lib.gen_fontstring = function(f, name, size, thinoutline)
	local fs = f:CreateFontString(nil, "OVERLAY")
	fs:SetFont(name, size, thinoutline)
...
lib.addHealthBar = function(f)
	...
	if f.mystyle=="boss" then
		...
		elseif f.mystyle == "pet"or f.mystyle == "focus" or f.mystyle == "focustarget" or f.mystyle == "tot" then
		s:SetHeight(26)
		s:SetWidth(f:GetWidth())
		s:SetPoint("BOTTOMLEFT", f, "BOTTOMLEFT", 0, 0)
		s:SetStatusBarTexture(cfg.statusbar_texture)
		...
		--s:SetHeight(retVal(f,f:GetHeight()*.6,f:GetHeight()*.76,29))
		--s:SetWidth(f:GetWidth())
		--s:SetPoint("TOP",0,0)
...
lib.addStrings = function(f)
	...
	if f.mystyle == "boss" then
		...
		altppval = lib.gen_fontstring(f.Health, cfg.font, 14, "THINOUTLINE")
		...
		name = lib.gen_fontstring(f.Health, retVal(f,cfg.font,cfg.font,cfg.raidfont), retVal(f,16,14,14), retVal(f,"THINOUTLINE","THINOUTLINE","THINOUTLINE"))
		name:SetPoint("LEFT", f.Health, "LEFT", retVal(f,5,3,1), retVal(f,0,0,0))
		name:SetJustifyH("LEFT")
		name.frequentUpdates = true
		hpval = lib.gen_fontstring(f.Health, cfg.font, retVal(f,15,12,14), retVal(f,"THINOUTLINE","THINOUTLINE","THINOUTLINE"))
		hpval:SetPoint(retVal(f,"RIGHT","RIGHT","LEFT"), f.Health, retVal(f,"RIGHT","RIGHT","BOTTOMLEFT"), retVal(f,-3,-3,0), retVal(f,-1,0,6))
		hpval.frequentUpdates = true
		powerval = lib.gen_fontstring(f.Health, cfg.font, 12, "NONE")
		powerval:SetPoint("TOP", f.Health, "BOTTOM", 0, -2)
		if f.mystyle == "raid" then
			...
			if f.mystyle == "player" then
				--f:Tag(name, "[drk:color][drk:power]|r[drk:afkdnd]")
					f:Tag(name, "[drk:color]|r[drk:afkdnd]")
...
lib.addAltPowerBar = function(f)
	...
	s:SetFrameLevel(1)
	if f.mystyle == 'boss' then
		s:SetPoint("BOTTOM", f.Power, "TOP", 0, 4)
		s:SetSize(f:GetWidth()-40, 3)
	else
...
lib.addPortrait = function(f)
    ...
    p:SetHeight(0.01)
    p:SetWidth(0.01)
    p:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", -5, -5)
    --helper
...
lib.addInfoIcons = function(f)
	...
	if f.mystyle == 'player' then
		f.PvP:SetHeight(15)
		f.PvP:SetWidth(15)
		f.PvP:SetPoint("RIGHT", 7, 2)
	elseif f.mystyle == 'target' then
		f.PvP:SetHeight(15)
		f.PvP:SetWidth(15)
		f.PvP:SetPoint("RIGHT", 6, 2)
	end
	-- Rest Icon
    if f.mystyle == 'player' then
		f.Resting = h:CreateTexture(nil, 'OVERLAY')
		f.Resting:SetSize(20,20)
		f.Resting:SetPoint('BOTTOMRIGHT', -12, -20)
   ...
   --LFDRole icon
	if f.mystyle == 'player' or f.mystyle == 'target' then
		...
		f.LFDRole:SetPoint('BOTTOMLEFT', -3, -4)
	...
	if f.mystyle ~= 'raid' then
		...
		li:SetPoint("LEFT", f, -1, 5)
		li:SetSize(12,12)
		f.Leader = li
		ai = h:CreateTexture(nil, "OVERLAY")
		ai:SetPoint("LEFT", f, -1, 5)
		ai:SetSize(12,12)
		f.Assistant = ai
...
lib.addCastBar = function(f)
	...
	elseif f.mystyle=="boss" then
		s:SetPoint("TOP",f.Health,"BOTTOM",10,-4)
		s:SetHeight(16)
    ...
    --spell text
    local txt = lib.gen_fontstring(s, cfg.font, 15, "NONE")
    txt:SetPoint("LEFT", 8, 1)
    txt:SetJustifyH("LEFT")
    --time
    local t = lib.gen_fontstring(s, cfg.font, 13, "NONE")
...
local myPostCreateIcon = function(self, button)
	...
	button.time = lib.gen_fontstring(button, cfg.smallfont, 13, "THINOUTLINE")
	button.time:SetPoint("BOTTOM", button, 4, -2)
	button.time:SetJustifyH('CENTER')
	button.time:SetVertexColor(1,1,1)

	button.count = lib.gen_fontstring(button, cfg.smallfont, 12, "THINOUTLINE")
...
lib.addBuffs = function(f)
	...
	if f.mystyle == "player" then
		b:SetPoint("BOTTOMLEFT", f, "TOPLEFT", 2, -8)
...
lib.addDebuffs = function(f)
    ...
    b:SetHeight((b.size+b.spacing)*2)
    --b:SetWidth(f:GetWidth())
    b:SetWidth((b.size+b.spacing)*7)

	b:SetPoint("TOPLEFT", f, "TOPLEFT", 3 , 17)
...
lib.addFocusAuras = function(f)
    ...
    b:SetHeight(b.size)
    b:SetWidth(f:GetWidth())
	b:SetPoint("TOPLEFT", f, "BOTTOMLEFT", 1, -15)
	b.initialAnchor = "TOPLEFT"
	b["growth-x"] = "RIGHT"
	b["growth-y"] = "DOWN"
    b.PostCreateIcon = myPostCreateIcon
...
lib.addBossBuffs = function(f)
    ...
    b:SetHeight(b.size)
    b:SetWidth(f:GetWidth())
	b:SetPoint("BOTTOMLEFT", f, "TOPLEFT", 3, 17)
	b.initialAnchor = "BOTTOMLEFT"
	b["growth-x"] = "RIGHT"
	b["growth-y"] = "DOWN"
    b.PostCreateIcon = myPostCreateIcon
...
lib.addBossDebuffs = function(f)
    ...
    b:SetHeight(b.size)
    b:SetWidth(f:GetWidth())
	b:SetPoint("TOPLEFT", f, "BOTTOMLEFT", 3, -2)
	b.initialAnchor = "TOPLEFT"
	b["growth-x"] = "RIGHT"
	b["growth-y"] = "DOWN"
    b.PostCreateIcon = myPostCreateIcon
...
lib.addHolyPower = function(self)
	...
	for i = 1, 5 do
		...
		PaladinHolyPower[i]:SetHeight(7)
...
lib.addRunes = function(self)
	...
	for i= 1, 6 do
		...
		Runes[i]:SetStatusBarColor(0, 0.7, 1)
